...
let AuthService = class AuthService {
    ...
    async refreshTokens(refreshToken) {
        try {
            ...
            console.log(isMatch);
            const token = this.jwtService.sign({ id: student.id, email: student.email }, { secret: process.env.SECRET_KEY || 'secret-key', expiresIn: '1m' });
            return { token };
        }
        catch (err) {
    ...
    async studentGoogleLogin(idToken) {
        ...
        const refreshtoken = this.jwtService.sign({ id: student.id, email: student.email, }, { expiresIn: '2h' });
        return {
            success: true,
            message: 'Logged in successfully',
