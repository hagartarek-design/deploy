...
let AuthService = class AuthService {
    ...
    async studentGoogleLogin(idToken) {
        try {
            ...
            const token = this.jwtService.sign({ id: students.id, email: students.email }, { secret: process.env.SECRET_KEY, expiresIn: '2d' });
            const refreshtoken = this.jwtService.sign({ id: students.id, email: students.email }, { secret: process.env.REFRESH_SECRET, expiresIn: '5d' });
            const hashedrefreshtoken = await bcrypt.hash(refreshtoken, 10);
            await this.students.update(students.id, { refreshToken: hashedrefreshtoken });
            return {
                success: true,
    ...
    async googleLogin(idToken) {
        ...
        const token = this.jwtService.sign({ id: user.id, email: user.email }, { expiresIn: '1m' });
        const refreshtoken = this.jwtService.sign({ id: user.id, email: user.email }, { expiresIn: '2m' });
        console.log(token);
        return {
            success: true,
            message: 'Logged in successfully',
